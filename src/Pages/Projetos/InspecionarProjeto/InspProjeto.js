...
function InspProjeto() {
  ...
  function deleteprojetos(id_projeto) {
    ...
    setprojetos(
      projetos.filter((projetos) => projetos.id_projeto !== id_projeto)
    );
  }
  ...
  function PutStatusproj() {
    ...
    const putprojstatus = () => {
      api
        .put(`/projetos/${id_projeto}/status/`, { status: statucucl })
        .then(() => {
          setOpenst(false);
    ...
    if (
      totalTaskAfazer === 0 &&
      totalTaskEmdesenvolvimento === 0 &&
      getstatus != "Concluído" &&
      totalTaskConcluído != 0
    ) {
      return (
        <>
          <Modal
          ...
          >
            <Box sx={style}>
              ...
              <Typography id="modal-modal-description" sx={{ mt: 2 }}>
                <div>
                  <button
                    className="btn-modal-put-status"
                    onClick={putprojstatus}
                  >
                    {" "}
                    Sim{" "}
                  </button>
  ...
  const filtFazendo = gettask.filter(
    (get) => get.status === "Em desenvolvimento"
  );
  ...
  function PutStatus() {
    if (valutask !== "") {
      api.put(`/tasks/${getid}/status`, { status: valutask });
        ...
        >
          <Box sx={style}>
              ...
              <div className="fields">
                ...
                <Box sx={{ minWidth: 120 }}>
                  <FormControl fullWidth>
                    ...
                    >
                      <MenuItem value={"facil"} key={"facil"}>
                        facil
                      </MenuItem>
                      <MenuItem value={"medio"} key={"medio"}>
                        medio
                      </MenuItem>
                      <MenuItem value={"dificil"} key={"dificil"}>
                        dificil
                      </MenuItem>
                    </Select>
            ...
            <div className="div-btn-modalEdit">
              <Button variant="outlined" onClick={handleClosen}>
                Cancelar
              </Button>
              <Button variant="outlined" onClick={Putdetask}>
                Editar
              </Button>
            </div>
          </Box>
  ...
  function Modaldeinformaçoes() {
    ...
    const datatesk = getpeople.map((getdata) =>
      getdata.created_at.slice(0, 10)
    );
      ...
      <>
        ...
        >
          <Box sx={style}>
            ...
            <Typography>
              <div>
                <button className="btn-novo-style" onClick={Deletetask}>
                  {" "}
                  Deletar{" "}
                </button>
                <button className="btn-novo-style" onClick={Editetask}>
                  {" "}
                  Editar{" "}
                </button>
              </div>
                      ...
                      <div id="cardtsk">
                        <div>
                          <p style={{ fontSize: "8px" }}>
                            {Datanew(pegacoment.created_at)}
                          </p>
                          <h6> {pegacoment.comentario}</h6>
                        </div>
                        <button
                          onClick={() =>
                            Deletetaskcoment(pegacoment.id_comentario)
                          }
      ...
      return (
        <h2>
          <img
            ...
            alt=" "
            width={"53%"}
            style={{ marginLeft: "28%", marginTop: "50%" }}
          />
        </h2>
      );
    } else {
                  ...
                  <div className="menu-dos-filtros-statusTask">
                    <div className="menu-dos-filtros-statusTask">
                      ...
                      >
                        ...
                        <Card.Text className="header-task-mudastatus">
                          {projetos.status}
                        </Card.Text>
                      </button>
                    </div>
                    <AiOutlineMore
                      onClick={() => lol(projetos.id_task)}
                      className="cor-menu-pontos"
                    />
                  </div>
                  <Card.Body>
                    <Card.Title className="name-task-inpprojeto" key={key}>
                      {projetos.descricao_task}
                    </Card.Title>
                    <Card.Title className="name-task-inpprojeto">
                      <span style={{ color: MudacorNivel(projetos.nivel) }}>
                        {projetos.nivel}
                      </span>
                    </Card.Title>
                    <div>
                ...
                <Card id="card-concl" className="card-color">
                  <div className="menu-dos-filtros-statusTask">
                    <div className="menu-dos-filtros-statusTask">
                      ...
                      >
                        <div className="btn-muda-status"></div>
                        <Card.Text className="header-task-mudastatus-conclu">
                          {projetos.status}
                        </Card.Text>
                      </button>
                    </div>
    ...
    <div>
      ...
      <div>
        <div>
          ...
          <p
            style={{ color: "#fff", marginBottom: "2rem", marginLeft: "50px" }}
          >
            <span style={{ color: MudacorStatus(projetos.status) }}>
              {projetos.status}
            </span>
          </p>
        </div>
      ...
      <div id="botoes-page-inp-projetos">
        ...
        <div className="btn-excluir-pagina-projeto">
          <button
            onClick={() => deleteprojetos(projetos.id_projeto)}
            aria-label="share"
            type="submit"
            to="/pessoas"
          >
      ...
      <div className="d-flex ">
        <div
          ...
          style={{ height: "750px", padding: "2rem" }}
        >
          <div
          ...
          <div
            ...
            id="BarraRolage"
          >
            <div id="header-status-desenv">Em desenvolvimento</div>
          ...
          <div
            ...
            id="BarraRolag"
          >
            <div id="header-status-con">Concluídos</div>
            <VerificaConcluído />
            <PutStatusproj />
          </div>
        </div>
